...
class Particle {
  ...
  update(width: number, height: number, mouse: { x: number, y: number, radius: number }, particles: Particle[], parallaxOffset: number) {
    this.angle += 0.01;
    this.vx += Math.cos(this.angle) * 0.002;
    this.vy += Math.sin(this.angle) * 0.002;

    // IMPORTANT: Calculate current screen Y for accurate interaction
    const screenY = (this.y - parallaxOffset + height) % height;
    ...
    if (repulsionMultiplier > 0) {
      for (let i = 0; i < particles.length; i++) {
        ...
        const p2ScreenY = (p2.y - parallaxOffset + height) % height;
        const rdy = screenY - p2ScreenY;
...
const NeuralBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const drawConnections = (parallaxOffset: number) => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          const organicOpacity = Math.pow(proximity, 4) * 0.22 * breathing;
    ...
    const animate = () => {
      ...
      particles.forEach(p => {
        p.update(canvas.width, canvas.height, mouse, particles, parallaxOffset);
